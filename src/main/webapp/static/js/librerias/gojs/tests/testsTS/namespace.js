...
(function () {
    ...
    nsTests.add(new Test("check module namespace", null, function (test) {
        const diag = test.diagram;
        const ns = gojs; // what was imported
        const go2 = ns.go; // the real "go"
        ...
        let nsonly = "";
        for (let p of (nssyms.copy().removeAll(go2syms)))
            nsonly += p + " ";
        test.assert(nsonly === "", "ns only: " + nsonly);
        let go2only = "";
        for (let p of (go2syms.copy().removeAll(nssyms)))
            go2only += p + " ";
