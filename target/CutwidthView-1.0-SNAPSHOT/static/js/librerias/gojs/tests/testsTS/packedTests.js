...
import { PackedLayout, PackMode, PackShape, SortMode, SortOrder, } from '../../extensionsJSM/PackedLayout.js';
...
(function () {
    ...
    function NoIntersectionsCheck(test) {
        const it = test.diagram.nodes;
    ...
    function NodePositionsCheck(test, positions) {
        const it = test.diagram.nodes;
        while (it.next()) {
            const n = it.value;
            const pos = positions[n.key];
            test.assert(n.actualBounds.x === pos.x && n.actualBounds.y === pos.y, 'Position for ' + n.key + ' should be ' + pos + ', not ' + n.actualBounds.position);
        }
    }
    function SetupGraph(test, genParams, layoutParams, nodeSizes) {
        if (!genParams)
            genParams = { numNodes: 0, minSide: 0, maxSide: 0 };
        // define the Node template
        const $ = go.GraphObject.make;
        test.diagram.nodeTemplate = $(go.Node, 'Auto', $(go.Shape, { strokeWidth: 0 }, new go.Binding('figure', 'figure'), new go.Binding('width', 'width'), new go.Binding('height', 'height'), new go.Binding('fill', 'fill')), $(go.TextBlock, { font: '10pt sans-serif' }, new go.Binding('text', 'key'), new go.Binding('stroke', 'textColor'), new go.Binding('font', 'textFont')), {
            toolTip: $(go.Adornment, 'Auto', $(go.Shape, { fill: '#FFFFCC' }), $(go.TextBlock, { margin: 4 }, new go.Binding('text', 'key'))), // end of Adornment
        });
    ...
    packedTests.add(new Test('Aspect ratio', null, function (test) {
        SetupGraph(test, {
            numNodes: 100,
            minSide: 40,
            maxSide: 60,
        ...
        const actualAspectRatio = test.diagram.documentBounds.width / test.diagram.documentBounds.height;
        test.assert(Math.round(actualAspectRatio) === 3, 'Actual aspect ratio should be 3, not ' + actualAspectRatio);
    }));
    const fixedSizeAcceptableError = 60;
    packedTests.add(new Test('Fit (compress)', null, function (test) {
        test.width = 200;
        test.height = 400;
        SetupGraph(test, {
            numNodes: 100,
        ...
        test.assert(width > test.width - fixedSizeAcceptableError &&
            width < test.width + fixedSizeAcceptableError, 'Width ' + width + ' is not within acceptable range of ' + test.width);
        test.assert(height > test.height - fixedSizeAcceptableError &&
            height < test.height + fixedSizeAcceptableError, 'Height ' + height + ' is not within acceptable range of ' + test.height);
    }));
    packedTests.add(new Test('Fit (expand)', null, function (test) {
        test.width = 600;
        test.height = 800;
        SetupGraph(test, {
            numNodes: 100,
        ...
        AllNodesLocatedCheck(test);
        NoIntersectionsCheck(test);
        test.assert(test.diagram.layout._fixedSizeModeSpacing !== undefined, 'Cannot find _fixedSizeModeSpacing property on PackedLayout');
        test.assert(test.diagram.layout._fixedSizeModeSpacing === 0, '_fixedSizeModeSpacing should be 0, packMode.Fit should not ever cause the layout to expand');
    }));
    packedTests.add(new Test('ExpandToFit (compress)', null, function (test) {
        test.width = 300;
        test.height = 400;
        SetupGraph(test, {
            numNodes: 100,
    ...
    packedTests.add(new Test('ExpandToFit (expand)', null, function (test) {
        test.width = 800;
        test.height = 1000;
        SetupGraph(test, {
            numNodes: 100,
    ...
    packedTests.add(new Test('ExpandToFit (expand to viewport)', null, function (test) {
    ...
    packedTests.add(new Test('sortMode none', null, function (test) {
        SetupGraph(test, null, {
    ...
    packedTests.add(new Test('sortMode max side', null, function (test) {
        SetupGraph(test, null, {
    ...
    packedTests.add(new Test('sortMode area', null, function (test) {
        SetupGraph(test, null, {
    ...
    packedTests.add(new Test('sortOrder ascending', null, function (test) {
        SetupGraph(test, null, {
            sortMode: SortMode.Area,
    ...
    packedTests.add(new Test('Custom comparer', null, function (test) {
        SetupGraph(test, null, {
            sortMode: SortMode.Area,
            comparer: (a, b) => {
    ...
    packedTests.add(new Test('packShape Elliptical', null, function (test) {
        SetupGraph(test, {
            numNodes: 21,
            minSide: 40,
            maxSide: 40,
    ...
    packedTests.add(new Test('packShape Rectangular', null, function (test) {
        SetupGraph(test, {
            numNodes: 16,
            minSide: 40,
            maxSide: 40,
    ...
    packedTests.add(new Test('Enclosing circle around rectangles', null, function (test) {
        SetupGraph(test, {
            numNodes: 30,
            minSide: 40,
            maxSide: 40,
        ...
        const x = -121.735;
        const y = -129.235;
        test.assert(round(c.width) === d && round(c.height) === d, 'Enclosing circle size not correct, should be ' +
            d +
            ' instead of ' +
            round(c.width) +
            ', ' +
            round(c.height));
        test.assert(round(c.x) === x && round(c.y) === y, 'Enclosing circle position not correct, should be ' +
            x +
            ', ' +
            y +
            ' instead of ' +
    ...
    packedTests.add(new Test('Enclosing circle around circles', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 30,
            minSide: 30,
    ...
    rectanglePackingTests.add(new Test('Zero nodes', null, function (test) {
        SetupGraph(test, {
            numNodes: 0,
            minSide: 40,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('One node', null, function (test) {
        SetupGraph(test, {
            numNodes: 1,
            minSide: 40,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('Two nodes', null, function (test) {
        SetupGraph(test, {
            numNodes: 2,
            minSide: 40,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('Three nodes', null, function (test) {
        SetupGraph(test, {
            numNodes: 3,
            minSide: 40,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('Basic pregenerated', null, function (test) {
    ...
    rectanglePackingTests.add(new Test('500 nodes', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 40,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('500 wide nodes', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 40,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('500 wide or tall nodes', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 40,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('Nodes with 0 width/height', null, function (test) {
        SetupGraph(test, {
            numNodes: 25,
            minSide: 0,
            maxSide: 2,
    ...
    rectanglePackingTests.add(new Test('Odd starting shape', null, function (test) {
        SetupGraph(test, null, undefined, [
            new go.Size(12, 40),
            new go.Size(50, 10),
            new go.Size(50, 10),
    ...
    rectanglePackingTests.add(new Test('Slightly varied wide nodes', null, function (test) {
        const sizes = new Array(75);
        for (let i = 0; i < 50; i++) {
    ...
    rectanglePackingTests.add(new Test('Floating point stability', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 20,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('Positive spacing', null, function (test) {
        SetupGraph(test, null, {
            spacing: 10,
        }, [
            new go.Size(30, 50),
    ...
    rectanglePackingTests.add(new Test('Small negative spacing', null, function (test) {
        SetupGraph(test, null, {
            spacing: -5,
        }, [
            new go.Size(30, 50),
    ...
    rectanglePackingTests.add(new Test('Medium negative spacing', null, function (test) {
        SetupGraph(test, null, {
            spacing: -30,
        }, [
            new go.Size(30, 50),
    ...
    rectanglePackingTests.add(new Test('Huge negative spacing', null, function (test) {
        SetupGraph(test, null, {
            spacing: -1000,
        }, [
            new go.Size(30, 50),
    ...
    rectanglePackingTests.add(new Test('500 nodes with huge negative spacing', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 20,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('Extreme variation', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 1,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('Extreme variation #2', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 1,
            maxSide: 90,
    ...
    rectanglePackingTests.add(new Test('Extreme variation #3', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 1,
            maxSide: 120,
    ...
    rectanglePackingTests.add(new Test('Extreme variation (area sorted desc.)', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 1,
            maxSide: 60,
    ...
    rectanglePackingTests.add(new Test('Extreme variation (area sorted asc.)', null, function (test) {
        SetupGraph(test, {
            numNodes: 500,
            minSide: 1,
            maxSide: 60,
    ...
    circlePackingTests.add(new Test('Zero nodes', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 0,
            minSide: 40,
    ...
    circlePackingTests.add(new Test('One node', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 1,
            minSide: 40,
    ...
    circlePackingTests.add(new Test('Two nodes', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 2,
            minSide: 40,
    ...
    circlePackingTests.add(new Test('Three nodes', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 3,
            minSide: 40,
    ...
    circlePackingTests.add(new Test('500 nodes', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 500,
            minSide: 40,
    ...
    circlePackingTests.add(new Test('Nodes with 0 width/height', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 25,
            minSide: 0,
    ...
    circlePackingTests.add(new Test('Floating point stability', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 500,
            minSide: 30,
    ...
    circlePackingTests.add(new Test('Spiral packing', null, function (test) {
        test.circles = true;
        SetupGraph(test, null, {
    ...
    circlePackingTests.add(new Test('Positive spacing', null, function (test) {
        test.circles = true;
        SetupGraph(test, null, {
            spacing: 10,
            hasCircularNodes: true,
    ...
    circlePackingTests.add(new Test('Small negative spacing', null, function (test) {
        test.circles = true;
        SetupGraph(test, null, {
            spacing: -5,
            hasCircularNodes: true,
    ...
    circlePackingTests.add(new Test('Medium negative spacing', null, function (test) {
        test.circles = true;
        SetupGraph(test, null, {
            spacing: -40,
            hasCircularNodes: true,
    ...
    circlePackingTests.add(new Test('Huge negative spacing', null, function (test) {
        test.circles = true;
        SetupGraph(test, null, {
            spacing: -1000,
            hasCircularNodes: true,
    ...
    circlePackingTests.add(new Test('500 nodes with huge negative spacing', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 500,
            minSide: 20,
    ...
    circlePackingTests.add(new Test('Extreme variation', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 500,
            minSide: 1,
    ...
    circlePackingTests.add(new Test('Extreme variation (area sorted desc.)', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 500,
            minSide: 1,
    ...
    circlePackingTests.add(new Test('Extreme variation (area sorted asc.)', null, function (test) {
        test.circles = true;
        SetupGraph(test, {
            numNodes: 500,
            minSide: 1,
