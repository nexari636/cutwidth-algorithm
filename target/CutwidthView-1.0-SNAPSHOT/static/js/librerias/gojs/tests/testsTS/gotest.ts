 ...
 */
export class Test {
  public parent: Test | TestCollection | null;
  public name: string;
  public diagramId: string;
  public diagram: go.Diagram | null;
  public preSetup: ((t: Test) => void) | null;
  public setup: ((t: Test) => void) | null;
  public exec: ((t: Test) => void) | null;
  public check: ((t: Test) => void) | null;
  public animated: boolean;
  public savedEvent: go.DiagramEvent | null;

  constructor(
    name: string,
    diagram: go.Diagram | string | null,
    setup?: ((t: Test) => void) | null,
    execute?: ((t: Test) => void) | null,
    check?: ((t: Test) => void) | null
  ) {
    this.parent = null;
    this.name = name;
  ...
  getPath(): string {
    let str = this.name;
    let t = this as Test | TestCollection | null;
    while (t && t.parent) {
   ...
   */
  run(): void {
    if (TestCollection.LogTestName) this.log(this.getPath());
    ...
    if (this.diagramId !== '') {
      ...
      if (this.diagram === null) {
        ...
        if (this.diagram.div === null) {
          ...
          if (!div) div = document.getElementById('myDiagram');
          if (div instanceof HTMLDivElement) this.diagram.div = div;
        }
      }
      this.diagram.reset();
      ...
      while (currentParent && currentParent.parent) {
        ...
        if (currentParent.preSetup) currentParent.preSetup(this);
      }
    }
    if (TestCollection.Catch) {
      ...
      if (this.setup) this.setup(this);
      if (this.diagram !== null) this.diagram.maybeUpdate();
      if (this.exec) this.exec(this);
      if (this.diagram !== null) this.diagram.maybeUpdate();
      if (this.check) this.check(this);
    }
  ...
  error(msg: string): void {
    if (window) {
      ...
      this.log(str);
      if (TestCollection.Alerts) window.alert(str);
    }
  }
   ...
   * @param {string} msg
   */
  assert(pred: boolean, msg: string): void {
    if (!pred) {
      this.error(msg);
    }
  }
  ...
  dumpAllNodeLocations(): void {
    ...
    for (let i = 0; i < datas.length; i++) {
      const node = diagram.findPartForData(datas[i]) as go.Part;
      if (!node) continue;
  ...
  dumpLinkPoints(): void {
    ...
    if (!diagram) return;
    if (window.console) {
      const datas = (diagram.model as go.GraphLinksModel).linkDataArray;
      for (let i = 0; i < datas.length; i++) {
        const link = diagram.findLinkForData(datas[i]) as go.Link;
        if (!link) continue;
  ...
  dumpLinkGeometries(): void {
    ...
    for (let i = 0; i < datas.length; i++) {
      const link = diagram.findLinkForData(datas[i]) as go.Link;
      if (link) {
        const msg = this.name + ' ' + (link.geometry as any).toString(1);
        window.console.log(msg);
      }
    }
  }
   ...
   */
  assertAllNodeLocations(ptarr: Array<go.Point>): void {
    if (!Array.isArray(ptarr)) throw new Error('first arg is not an Array: ' + ptarr);
    if (!this.diagram) return;
    const model = this.diagram.model;
    const datas = model.nodeDataArray;
    for (let i = 0; i < ptarr.length; i++) {
      const expected = ptarr[i];
      const node = this.diagram.findNodeForData(datas[i]) as go.Part;
      const actual = node.location;
      this.assert(
        this.isApproxPoint(expected, actual),
        'Node is at location ' + actual + ' instead of at ' + expected
      );
    }
  }
   ...
   */
  assertAllLinkPoints(testArrayPoints: Array<Array<go.Point>>): void {
    ...
    for (let i = 0; i < datas.length; i++) {
      ...
      if (link) this.assertLinkPoints(link, testArrayPoints[i]);
    }
  }
   ...
   */
  assertLinkPoints(link: go.Link, testPoints: Array<go.Point>): void {
    if (!(link instanceof go.Link)) throw new Error('first arg is not a link: ' + link);
    const linkPoints = link.points;
    if (!linkPoints || !testPoints) return;
    this.assert(
      linkPoints.length === testPoints.length,
   ...
   */
  assertAllLinkGeometries(testArrayGeometries: Array<string>): void {
    ...
    for (let i = 0; i < datas.length; i++) {
      ...
      const link = this.diagram.findLinkForData(data);
      this.assert(link !== null, 'Did not find Link for link data: ' + data);
      if (link) this.assertLinkGeometry(link, testArrayGeometries[i]);
    }
  }
   ...
   */
  assertLinkGeometry(link: go.Link, testGeom: go.Geometry | string): void {
    const linkgeo = link.geometry;
    const geomstr = (linkgeo as any).toString(1);
    const teststr = typeof testGeom === 'string' ? testGeom : (testGeom as any).toString(1);
    ...
    this.assert(
      same,
      'The geometry of ' + link + ' is incorrect. Got: ' + geomstr + '.\n  Expected: ' + teststr
    );
  }
   ...
   * @this {Test}
   */
  pathToPointArray(geom: go.Geometry): Array<go.Point> | null {
    if (geom.type !== go.Geometry.Path) return null;
    const arr = [];
      ...
      while (segments.next()) {
        ...
        switch (seg.type) {
          ...
          case go.PathSegment.SvgArc: {
            const curves =
              seg.type === go.PathSegment.Arc
                ? (seg as any).buildBeziers(fig)
                : (seg as any).buildSVGBeziers(fig, lastpointX, lastpointY);
   ...
   * @return {boolean}
   */
  isApproxEqual(x: number, y: number): boolean {
    const d = x - y;
   ...
   * @return {boolean}
   */
  isApproxPoint(a: go.Point, b: go.Point): boolean {
   ...
   * @return {boolean}
   */
  isApproxSize(a: go.Size, b: go.Size): boolean {
   ...
   * @return {boolean}
   */
  isApproxRect(a: go.Rect, b: go.Rect): boolean {
    return (
      this.isApprox(a.x, b.x) &&
   ...
   */
  mouseDown(pt: go.Point, eventprops: any, optionalDiagram?: go.Diagram): void {
    let diagram = optionalDiagram || this.diagram;
    if (!diagram || !diagram.isEnabled) return;
   ...
   * @param {object=} eventprops an optional argument providing properties for the InputEvent.
   */
  mouseMove(pt: go.Point, eventprops: any, optionalDiagram?: go.Diagram): void {
    let diagram = optionalDiagram || this.diagram;
    if (!diagram || !diagram.isEnabled) return;
    if (eventprops && eventprops.sourceDiagram) diagram = eventprops.sourceDiagram;
    if (!diagram || !diagram.isEnabled) return;
   ...
   * @param {object=} eventprops an optional argument providing properties for the InputEvent.
   */
  mouseUp(pt: go.Point, eventprops: any, optionalDiagram?: go.Diagram): void {
    let diagram = optionalDiagram || this.diagram;
    if (!diagram || !diagram.isEnabled) return;
    if (eventprops && eventprops.sourceDiagram) diagram = eventprops.sourceDiagram;
    if (!diagram || !diagram.isEnabled) return;
   ...
   * @param {object=} eventprops an optional argument providing properties for the InputEvent.
   */
  mouseWheel(delta: number, eventprops: any): void {
    const diagram = this.diagram;
    if (!diagram || !diagram.isEnabled) return;
   ...
   * @param {object=} eventprops an optional argument providing properties for the InputEvent.
   */
  keyDown(keyorcode: string | number, eventprops: any): void {
    const diagram = this.diagram;
    if (!diagram || !diagram.isEnabled) return;
    const n = diagram.lastInput.copy();
    n.diagram = diagram;
   ...
   * @param {object=} eventprops an optional argument providing properties for the InputEvent.
   */
  keyUp(keyorcode: string | number, eventprops: any): void {
    const diagram = this.diagram;
    if (!diagram || !diagram.isEnabled) return;
    const n = diagram.lastInput.copy();
    n.diagram = diagram;
...
export class TestCollection {
  public parent: TestCollection | null;
  public name: string;
  public tests: go.List<Test>;
  public collections: go.List<TestCollection>;
  public preSetup: ((t: Test) => void) | null;
   ...
   */
  add(test: Test | TestCollection): void {
   ...
   * @param {Test|TestCollection} test
   */
  addAnimated(test: Test | TestCollection): void {
    if (test instanceof Test) {
      test.parent = this;
      test.animated = true;
      this.tests.add(test);
   ...
   */
  find(name: string): Test | TestCollection | null {
   ...
   * @return {Test}
   */
  findTest(name: string): Test | null {
    const tit = this.tests.iterator;
    while (tit.next()) {
      const t = tit.value;
      if (t.name === name) return t;
  ...
  static run(): void {
    if (window) {
      if (TestCollection.Queue.length > 0) {
        ...
        let time = 1;
        if (TestCollection.Current !== null && TestCollection.Current.animated) time = 100;
        ...
        let num = '';
        if (TestCollection.TotalNumber) num = TestCollection.TotalNumber.toString();
        let errs = '';
   ...
   * @return {Test}
   */
  static findTest(path: string): Test | TestCollection | null {
    const split = path.split('|');
    if (split.length === 1) {
      // search for a Test, anywhere in tree
      return TestRoot.findTest(split[0]);
    } else {
      let t: Test | TestCollection = TestRoot;
      // ignore initial '|'
      for (let i = 1; i < split.length; i++) {
        const n = split[i];
        const c: Test | TestCollection | null = t instanceof TestCollection ? t.find(n) : null;
  ...
  static GlobalDiagram: go.Diagram = new go.Diagram();

  static StatusId: string = 'myStatus';

  /**
   * The list of Tests to be run.
   * @type {List}
   */
  static Queue: go.List<Test> = new go.List(/*Test*/);

  /**
   * Gets or sets whether very slow tests are run
   * @type {boolean}
   */
  static RunSlowTests: boolean = false;

  /**
   * Gets or sets whether Test.run logs the name of the test.
   * @type {boolean}
   */
  static LogTestName: boolean = true;

  /**
   * Gets or sets whether Test.error() will show an alert().
   * @type {boolean}
   */
  static Alerts: boolean = true;
   ...
   * @type {boolean}
   */
  static Catch: boolean = true;

  /**
   * Gets the currently running Test, if any.
   * @type {Test}
   */
  static Current: Test | null = null;

  // variables holding test results:

  static StartTime: number | undefined = Date.now();
   ...
   * @type {number}
   */
  static TotalErrors: number = 0;

  /**
   * Keep track of how many warnings occurred during runAll.
   * @type {number}
   */
  static TotalWarnings: number = 0;
 ...
 * @type {TestCollection}
 */
export var TestRoot: TestCollection = new TestCollection('TestRoot');

// Initialize the tree of tests
var myTreeDiagram: go.Diagram;

export function initTree(divid: any, statusid: string): go.Diagram {
  ...
  myTreeDiagram.nodeTemplateMap.add(
    ...
    $(
      go.Node,
      go.Panel.Horizontal,
      $(go.TextBlock, new go.Binding('text', '', go.Binding.toString), {
  ...
  myTreeDiagram.nodeTemplateMap.add(
    '', // Tests don't have any tree children
    $(
      go.Node,
      go.Panel.Horizontal,
      $(go.TextBlock, new go.Binding('text', '', go.Binding.toString), {
...
function walkTests(tc: any /*TestCollection*/, nodes: Array<any>, links: Array<any>): void {
  if (!tc) return;
...
function collapseTree(levels: number) {
